...
| Finding | Evidence |
|---|---|
| **A ship that arrives climbing scores more than a perfect stop, and one pad can be reached that way.** The bonus is an 8-bit `80 − velocity`. Poked at −16 on the x10 pad it prints `960 X 10= 9600`; flown, with stick inputs only from a hover beside the x2 plateau, it prints `1270 X  2= 2540`, the routine's ceiling for that pad. A perfect stop is unreachable from above on any pad. See `facts.md`, Reachable landings | `touchdown` `$E581`; `check_on_pad` `$E3B5`; two input movies replayed live, every pass matching the model |
