...
| **The ending**: a congratulation, "save the status for Mercenary II", then "GAME OVER" shown for ever; the game goes on (Zzap 13, leaflet) | traced | script 7 and the starfield `$AA62`, which never returns; `$BEC0` bit 6 is never cleared, so GAME OVER repeats every 15 s |
