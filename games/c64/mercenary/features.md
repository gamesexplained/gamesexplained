...
|---|---|---|
| **Opening sequence** (Zzap 11): the ship sits in space before a starfield; Benson engages the Novadrive, a fault develops, even maximum reverse thrust cannot stop the crash on Targ; then full manual control | live | the intro script at `$7001` (poked over a countdown, "ABC UNLIMITED", "NOVADRIVE ENGAGED", "PRESTINIUM ON COURSE", "DESTINATION GAMMA FIVE", "DAMAGE CONTROL REPORT", "GUIDANCE SYSTEM FAULT", "COLLISION COURSE", "UNABLE TO CORRECT", "MAXIMUM REVERSE THRUST"); the starfield `$AA62`; about 75 s from `$5000` to standing at `08-08` |
...
| **Buying the Dominion Dart** for 5000 credits, or stealing it (Zzap 11; 5000 and a 9000 start on another machine, Electron Dance) | live (the offer) | "DO YOU WANT TO BUY?" appeared as the player walked up; script 1 at `$0A3C` holds the question, the test for Y at `$0AB6`, and "THIS CRAFT BELONGED TO THE PALYAR COMMANDER'S BROTHER-IN-LAW / HE IS NOT TOO PLEASED". A Y press did not buy it in the one attempt made |
| **Idle messages** when the player does nothing after the crash (Zzap 13, site) | traced | script 0: "IS ANYBODY THERE", "AM I TALKING TO MYSELF", "ARE YOU STILL ALIVE", "WHERE ARE YOU", "AH, YOU ARE BACK" |
...
- **It refuses NTSC.** `$5024` reads the KERNAL's PAL flag `$02A6` and, on an
  NTSC machine, loops back to `$5000` for ever.
- **RESTORE does nothing**: the NMI vector `$8009` is a bare `RTI`.
